...
{
    const updateHtml = await CreateHTMLForAllUpdates(allUpdateTables);
...
/**
 * @param {UpdateTables[]} allUpdateTables 
 * @returns {Promise<AllUpdates>}
 */
async function CreateHTMLForAllUpdates(allUpdateTables)
{
    ...
    if (userConfig?.rateNoticeDate && new Date() > userConfig?.rateNoticeDate) {
        divAllUpdates.append(CreateRateNoticeDiv());        
    }
...
function CreateRateNoticeDiv()
{
    const rateDiv = document.createElement('div');
    rateDiv.classList.add('single-update-tile');
    rateDiv.classList.add('tile-max-width-from-config');
    rateDiv.classList.add('rate-tile');
    rateDiv.innerHTML = `
        This history was generated by the open source 
        <a href="https://github.com/Sedeniono/ADO-History-Diff" target="_blank" rel="noopener noreferrer">History Diff</a> extension.
        <br>
        <b>If you find it useful, please consider giving the project a star on 
        <a href="https://github.com/Sedeniono/ADO-History-Diff" target="_blank" rel="noopener noreferrer">GitHub</a> 
        and leaving a rating on the 
        <a href="https://marketplace.visualstudio.com/items?itemName=Sedenion.HistoryDiff" target="_blank" rel="noopener noreferrer">Marketplace</a>!</b>`;
    
    const rateButtonsDiv = document.createElement('div');
    rateButtonsDiv.style.paddingTop = '5px';

    const remindRateButton = document.createElement('button');
    remindRateButton.id = 'remind-rate-button';
    remindRateButton.innerText = 'Remind me in 5 days';
    remindRateButton.onclick = () => {
        // 5 days, 4 hours in the future: Don't choose exactly 7 days in case the message appears during a weekly meeting;
        // it would appear again during the same meeting one week later. Similar, don't choose an integral number of days in
        // case of daily meetings.
        // (We also ignore daylight saving time changes; one hour more or less doesn't matter.)
        const weekInMs = (5 * 24 + 4) * 60 * 60 * 1000;
        SetRateNoticeDate(new Date(Date.now() + weekInMs));
        rateDiv.remove();
    };

    const hideRateButton = document.createElement('button');
    hideRateButton.id = 'hide-rate-button';
    hideRateButton.style.marginLeft = '5px';
    hideRateButton.innerText = 'Never show this again';
    hideRateButton.onclick = () => {
        // 999 years (apart from leap years and such, which obviously are irrelevant for that purpose).
        const manyYearsInMs = 999 * 365 * 24 * 60 * 60 * 1000;
        SetRateNoticeDate(new Date(Date.now() + manyYearsInMs));
        rateDiv.remove();
    };

    rateButtonsDiv.append(remindRateButton, hideRateButton);
    rateDiv.append(rateButtonsDiv);
    return rateDiv;
}
